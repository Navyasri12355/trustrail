...
def get_merchant_from_header(
    ...
    merchant = (
        db.query(Merchant)
        .filter(Merchant.merchant_id == x_merchant_id, Merchant.active == True)
        .first()
    )

    if not merchant:
        raise HTTPException(
            status_code=404, detail=f"Merchant {x_merchant_id} not found or inactive"
        )

    return merchant


def get_merchant_from_path(merchant_id: str, db: Session = Depends(get_db)) -> Merchant:
    """
    ...
    merchant = (
        db.query(Merchant)
        .filter(Merchant.merchant_id == merchant_id, Merchant.active == True)
        .first()
    )

    if not merchant:
        raise HTTPException(
            status_code=404, detail=f"Merchant {merchant_id} not found or inactive"
        )
