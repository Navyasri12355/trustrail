...
def _row_hash(prev_hash: str, mandate_id: str, event_type: str,
              decision: Optional[str], created_at: datetime) -> str:
    """
    SHA-256( prev_hash | mandate_id | event_type | decision | iso_timestamp ).
...
def log_guardrail_decision(
    ...
    rh = _row_hash(prev_hash, mandate_id, event_type, dec_str, now)
...
def log_mandate_event(
    ...
    rh = _row_hash(prev_hash, mandate_id, event_type, None, now)
